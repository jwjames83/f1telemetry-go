...
for multiplayer games where it is not always possible to send lap times on the
final frame because of network delay.
...
const ClassificationPacketSize = 839
const ResultsPacketSize = ClassificationPacketSize
...
type FinalClassificationData struct {
	...
	TotalRaceTime    double   // Total race time in seconds without penalties
...
type FinalClassification struct {
	...
	ClassificationData [MaxDrivers]FinalClassificationData
}

type Results = FinalClassification
